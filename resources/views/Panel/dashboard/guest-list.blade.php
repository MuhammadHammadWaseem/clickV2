...
<style>
    ...
    .guestlistmodal .modal-dialog .modal-header,
    .view_note_modal .modal-dialog .modal-header {
        padding: 40px 40px 0px;
    }

    .guestlistmodal .modal-dialog .modal-footer button,
    .view_note_modal .modal-dialog .modal-footer button {
    ...
    .view_note_modal .modal-content {
        background-color: #F1F1F1;
        border-radius: 10px;
        border: 1px solid #AAAAAA;
    }

    .guestlistmodal .modal-dialog .modal-footer,
    .view_note_modal .modal-dialog .modal-footer {
...
@section('content')
    ...
    <div class="modifier" id="modifier"
        ...
        <div class="button-group d-flex flex-column" style="gap:0.5rem;" id="modifierButton">
            ...
            <button class="btn btn-sm btn-primary undecline-btn" data-bs-toggle="modal"
                data-bs-target="#undeclineguestModal" onclick="undeclineGuest()"
                ...
                <div class="box-styling quick-actions">
                    ...
                    <div
                        ...
                        <p class="checkedincolor p-3" style="font-size:14px; color:rgb(37, 37, 37);" id="totalCheckIn">
                            <i class="fa fa-check-square" aria-hidden="true" style="color: rgb(37, 37, 37);"></i>
                            {{ __('guestlistpage.checked_in_c') }}
                        </p>
                        <p class="declinedcolor p-3" style="font-size:14px; color:rgb(37, 37, 37);" id="totalDecline">
                            <i class="fas fa-times" style="color: rgb(37, 37, 37);"></i>
                            {{ __('guestlistpage.declined_c') }}
                        </p>
                        <p class="checkedincolor p-3" style="font-size:14px; color:rgb(37, 37, 37);" id="totalinvite">
                            <i class="fas fa-user" style="color: rgb(37, 37, 37);"></i>
                            {{ __('guestlistpage.invite_guest_c') }}
                        </p>
                        <p class="checkedincolor p-3" style="font-size:14px; color:rgb(37, 37, 37);" id="inviteMembers">
                            <i class="fas fa-users" style="color: rgb(37, 37, 37);"></i>
                            {{ __('guestlistpage.invite_membrs_c') }}
                        </p>
                    </div>
                    <div class="text">
            ...
            <div class="col-lg-12">
                <div class="box-styling event-photos-gallery events-lists-sec-01 guest-list">
                    <div class="two-things-align">
                        ...
                        <div class="two-btn-align">
                            ...
                            <select onchange="showGuest(this.value)" class="form-select">
                                ...
                                <option value="not-sent">{{ __('guestlistpage.not_sent') }}</option>
                    ...
                    <div class="accordian-table-content">
                        ...
                        <div class="table-box">
                            <input type="text" id="guestSearchInput"
                                placeholder="{{ __('guestlistpage.search_main_guest') }}" class="form-control mb-3">
                            <table>
                                {{-- <tr>
                                    <th>Names</th>
                                    <th>Contact #</th>
                    ...
                    <form id="DisplaySaveOptionForm">
                        @if ($card)
                            <div class="text">
                                ...
                                <div class="form-group">
                                    <label for="attending">{{ __('guestlistpage.Attending') }}</label>
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="checkbox" name="go-to-the-website" id="go-to-the-website"
                                        {{ $card->rsvp[8] == 1 ? 'checked' : '' }}>
                                </div>
                                <div class="form-group">
                                    <label
                                        for="learn-how-RSVP-works">{{ __('guestlistpage.learn_how_rsvp_works') }}</label>
                                    <input type="checkbox" name="learn-how-RSVP-works" id="learn-how-RSVP-works"
                                        {{ $card->rsvp[10] == 1 ? 'checked' : '' }}>
                                </div>
                            </div>
                        @else
                            <div class="text">
        ...
        $(document).ready(function() {
            ...
            $('#guestSearchInput').on('keyup', function() {
                ...
                $('#GuestList .guest-list-card-main').each(function() {
                    ...
                    const mainGuestName = guestCard.find('.top-main-name-open span').text()
                        .toLowerCase().trim();
                    const guestEmail = guestCard.find('span i.fa-envelope-o').parent().text()
                        .toLowerCase();
                    const guestPhone = $(this).find('span svg.bi-telephone').parent().text()
                        .toLowerCase();
                    ...
                    const memberNames = guestCard
                        ...
                        .toLowerCase(); // Get and normalize text
                        })
                        .get();
                    ...
                    const memberMatch = memberNames.some((name) => name.includes(searchText)) ||
                        memberEmails.some((email) => email.includes(searchText)) || memberPhone
                        .some((phone) => phone.includes(searchText));
                    const isMainGuestMatch = mainGuestName.includes(searchText) || guestEmail
                        .includes(searchText) || guestPhone.includes(searchText);
                ...
                success: function(response) {
                    ...
                    var checkedInCount = guests.reduce(function(count, guest) {
                        return count + (guest.checkin == 1 ? 1 : 0) +
                            (guest.members ? guest.members.filter(member => member.checkin == 1)
                                .length : 0);
                    }, 0);
                    var declinedCount = guests.reduce(function(count, guest) {
                        return count + (guest.declined == 1 ? 1 : 0) +
                            (guest.members ? guest.members.filter(member => member.declined == 1)
                                .length : 0);
                    }, 0);
                    ...
                    $('#totalCheckIn').html(
                        `<i class="fa fa-check-square" aria-hidden="true"></i> {{ __('guestlistpage.checked_in_c') }} ${checkedInCount}`
                    );
                    $('#totalDecline').html(
                        `<i class="fa fa-times" aria-hidden="true"></i> {{ __('guestlistpage.declined_c') }} ${declinedCount}`);
                    $('#totalinvite').html(
                        `<i class="fa fa-user" aria-hidden="true"></i> {{ __('guestlistpage.invite_guest_c') }} ${totalGuests}`);
                    $('#inviteMembers').html(
                        `<i class="fa fa-users" aria-hidden="true"></i> {{ __('guestlistpage.invite_membrs_c') }} ${totalMembers}`);
                        ...
                        guests.forEach(function(guest) {
                                    ...
                                    <span data-toggle="modal" data-target="#exampleModalCenter${guest.id_guest}" >
                                        ...
                                        <strong>Status:</strong> ${
                                                    guest.declined == 1 && guest.opened != 2
                                                        ...
                                                        : guest.opened == null
                                                        ? "<span style='background-color:rgba(255, 0, 0, 0.3); color:#FF0000; border-radius:10px; padding:3px 10px; font-size:13px;'>Not Sent</span>"
                                                        : "Unknown"
                                                }
                            ...
                            guest.members.forEach(function(member) {
                                ...
                                <tr ${(member.declined == 1 ? 'style="background-color: #ffdbdb  !important;"' : '')} ${(member.checkin == 1 ? 'style="background-color: #dbffdb  !important;"' : '')}>
                                    ...
                                    ${(member.opened == 0) && member.declined != 1 && member.checkin != 1 ? `
                                                    <td class="accordian_img_acces">
                                                       <span style="background:rgba(255, 0, 0, 0.3); color:#FF0000;border-radius:10px; padding:10px; font-size:15px !important;">Not Open</span>
                                                    </td>` : ''}

                                    ${member.opened == null && member.declined != 1 && member.checkin != 1 ? `
                                                    <td class="accordian_img_acces">
                                                       <span style="background:rgba(255, 0, 0, 0.3); color:#FF0000; border-radius:10px; padding:10px; font-size:15px !important;">Not Sent</span>
                                                    </td>` : ''}
                    ...
                    if (filter == "checked-in") {
                        ...
                        guests.forEach(function(guest) {
                            ...
                            if (guest.members && guest.members.some(member => member.checkin == 1)) {
                                checkedInMembersCount += guest.members.filter(member => member
                                    .checkin == 1).length;
                            }
                        });
                        var br = '<br>';
                    ...
                    if (filter == "not-open") {
                        ...
                        guests.forEach(function(guest) {
                            ...
                            if ((guest.opened == 0) || guest.members.some(
                                    member => member.opened == 0)) {
                                if (guest.opened == 0) {
                                    notOpenedGuestsCount++; // Count not-open guest
                                }
                                if(guest.members.some(member => member.opened == 0)){
                                    notOpenedMembersCount += guest.members.filter(member => member.opened == 0).length;
                                }

                                notOpenedGuestsCount += guest.members.filter(member => member.opened == 0).length;
                                var br = '<br>';
                                $('#guestMemberTotal').html(
                    ...
                    if (filter == "not-sent") {
                        ...
                        guests.forEach(function(guest) {
                            // Check if guest or any member has opened == 0 or null (not open)
                            if ((guest.opened == null) || guest.members.some(
                                    member => member.opened == null)) {
                                if (guest.opened == null) {
                                    notOpenedGuestsCount++; // Count not-open guest
                                }
                                if(guest.members.some(member => member.opened == null)){
                                    notOpenedMembersCount += guest.members.filter(member => member.opened == null).length;
                                }

                                notOpenedGuestsCount += guest.members.filter(member => member.opened == null).length;
                                var br = '<br>';
                                $('#guestMemberTotal').html(
                                    `TOTAL: ${notOpenedGuestsCount} ${br} (guests ${notOpenedGuestsCount} - members ${notOpenedMembersCount})`
                                );
